...
class prismoide(figure):
    ...
    def replaceFace(self, f: face, i):
        self.faces[i] = f
        self.faces[i].setPos(
            point(f.position.x(), f.position.y(), f.position.z()))
    ...
    def getVolume(self, i1=0, i2=None):
        ...
        for i, face in enumerate(self.faces[i1:i2]):
            ...
            self.volume += (face.getArea()+nextFace.getArea()) * \
                abs(face.position.z()-nextFace.position.z())/2
    ...
    def getVolumes(self, i1=0, i2=None):
        try:
            ...
            for i, face in enumerate(self.faces[i1:i2]):
                ...
                ct, at = face.getAreas()
                vct += (ct + nct) * abs(face.position.z() -
                                        nextFace.position.z()) / 2
                vat += (at + nat) * abs(face.position.z() -
                                        nextFace.position.z()) / 2
        except:
...
def test1():
    ...
    x = curve()
    x.setPoints([point(-1, -1), point(1, -1),
...
def test2():
    terreno = [pt(-6, -5), pt(-4, -4), pt(-4, -2), pt(-3, 0), pt(-2, 2), pt(-1, 1),
               pt(0, 3), pt(1, 3), pt(2, 6), pt(3, 8), pt(4, 9), pt(5, 10), pt(6, 15)]
    tc = curve()
