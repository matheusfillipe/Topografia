...
class cv():
    ...
    def update(self, i1, i2, L, handlePos, lastHandlePos):
        if L != 0:
            ...
            self.curve.setData(x, y, pen=pg.mkPen(
                'r', width=3, style=QtCore.Qt.DashLine))
...
#        self.addCurveBtn.clicked.connect(self.raiseCurveGroupeBox)
       # self.setupUi(self)
        ...
        try:
            ...
            self.handle.curve = cv(self.i1, self.i2, 0,
                                   self.handle.pos(), self.getHandlePos(i-1))

        self.initialCurve = cv(self.handle.curve.i1, self.handle.curve.i2, self.handle.curve.L,
                               self.handle.curve.handlePos, self.handle.curve.lastHandlePos)
...
#       self.helpBtn.clicked.connect(self.displayHelp)
        ...
        self.shortcut1 = QtWidgets.QShortcut(
            QtGui.QKeySequence.MoveToNextChar, self)
    ...
    def displayHelp(self):
        dialog = imgDialog(imagepath="../view/ui/helpCV.png",
                           title="Ajuda: Curvas Verticais")
        dialog.show()
        dialog.exec_()
        ...
        try:
            if not self.isBeingModified:
                self.horizontal = float(
                    self.uihorizontal1.text())+self.getHandlePos(self.i-1).x()
                self.update()
                self.redefineUI(4)
                self.updateVerticesCb()
    ...
    def updateL(self):
        try:
            if not self.isBeingModified:
                ...
                self.handle.curve.update(self.i1, self.i2, self.Lutilizado, self.getHandlePos(
                    self.i), self.getHandlePos(self.i-1))
                self.roi.plotWidget.addItem(self.handle.curve.curve)
                self.updateAlert()
        except ValueError:
            pass
    ...
    def redefineUI(self, elm):
        ...
        Kdes = constants.Kdes[min(
            max(30, (round(velproj/10)*10)), 120)][self.G > 0]
...
class CustomPolyLineROI(pg.PolyLineROI):
    ...
    def setPoints(self, points, closed=None):
        ...
        QgsMessageLog.logMessage(
            "Iniciando pontos do perfil", "GeoRoad", level=0)
    ...
    def addCvs(self, cvList):
        ...
        if (cvList == False or len(cvList) <= 2):
            ...
            for i in range(0, len(self.handles) - 1):
                ...
                self.handles[i]['item'].sigRemoveRequested.connect(
                    self.removeCurva)
...
class ssRoi(CustomPolyLineROI):
    ...
    def setPoints(self, points, closed=None):
        ...
        for i in range(start, len(self.handles) - 1):
            self.addSegment(self.handles[i]['item'],
                            self.handles[i + 1]['item'])
            j = i + 1
            self.handles[j]['item'].sigEditRequest.connect(
                functools.partial(self.HandleEditDialog, j))
...
class Ui_Perfil(QtWidgets.QDialog):
    ...
    def __init__(self, ref_estaca, tipo, classeProjeto, greide, cvList, wintitle="Perfil Longitudinal", iface=None):
        ...
        self.iface = iface
        self.initVars(ref_estaca, tipo, classeProjeto,
                      greide, cvList, wintitle)
        self.setupUi(self)
        ...
        if self.greide:
           # i=0
            ...
            if xh != x[-1]:
                ...
                self.greide[-1] = final
                messageDialog(
                    message="O último vértice foi ajustado ao novo traçado!")
                L[-1] = final
    ...
    def updater(self):
        ...
        if not self.roi.ismodifying:
            handles = [self.roi.getHandlePos(i)
                       for i in range(self.roi.countHandles())]
    ...
    def calcularGreide(self):
        ...
        if p1 >= cfg.planoMin and p1 < cfg.planoMax:
            ...
            self.lblTipo.setText(u"Plano %s KM/h, Rampa n° %d, Inclinação %s%%" %
            ...
            self.lblTipo.setText(u"Ondulado %s KM/h, Rampa n° %d, Inclinação %s%%" %
                                 (s, maxIndex, roundFloat2str(I[maxIndex-1])[:-2]))
            ...
            if classeProjeto <= 0:
                ...
                s = "30"
            self.lblTipo.setText("Montanhoso %s KM/h, Rampa n° %d, Inclinação %s%%" %
                                 (s, maxIndex, roundFloat2str(I[maxIndex-1])[:-2]))

        else:
            s = "0"
            self.lblTipo.setText("Indefinido %s KM/h, Rampa n° %d, Inclinação %s%%" %
                                 (s, maxIndex, roundFloat2str(I[maxIndex-1])[:-2]))
        ...
        for a in A:
            ...
            if a == p1:
                if hasattr(self, "maxInclinationIndicatorLine"):
                    self.perfilPlot.removeItem(
                        self.maxInclinationIndicatorLine)
                    self.maxInclinationIndicatorLine.clear()
                    self.perfilPlot.update()
                self.maxInclinationIndicatorLine = pg.PlotCurveItem()
                self.maxInclinationIndicatorLine.setData([self.roi.getHandlePos(maxIndex-1).x(), self.roi.getHandlePos(maxIndex).x()], [
                                                         self.roi.getHandlePos(maxIndex-1).y(), self.roi.getHandlePos(maxIndex).y()], pen=pg.mkPen("r", width=4))
    ...
    def setupUi(self, PerfilTrecho):
        ...
        PerfilTrecho.setWindowTitle(_translate(
            "PerfilTrecho", "Perfil do trecho", None))
        self.calcularGreide()
        self.btnCalcular.setText("Calcular rampa máxima")

    def resetGeometry(self):
        reset = yesNoDialog(
            self, message="Realmente deseja excluir esse perfil?")
        if reset:
            self.reset.emit()
...
class Ui_sessaoTipo(Ui_Perfil):
    ...
    def __init__(self, iface, terreno, hor, ver=[], st=False, prism=None, estacanum=0, greide=[], title="Perfil Transversal"):
        ...
        if not st:
            ...
            defaultST = [[-9.549337802669857, -5.325013960480259], [-8.049337802669857, -5.370013960480259], [-4.299337802669858, -0.3700139604802599], [-4.099337802669858, -0.8700139604802597], [-3.699337802669858, -0.8700139604802597], [-3.499337802669858, -0.0700139604802598], [
                0.0006621973301417, -1.39604802598e-05], [3.500662197330141, -0.0700139604802598], [3.700662197330141, -0.8700139604802597], [4.100662197330142, -0.8700139604802597], [4.300662197330142, -0.3700139604802599], [6.800662197330142, 4.62998603951974], [8.300662197330142, 4.58498603951974]]
            ...
            for item in hor:
                ...
                for pt in defaultST:
                    newST.append(
                        [pt[0], pt[1]+self.verticais.getY(float(item[2]))])
                st.append(newST)
            self.st = st
        ...
        super(Ui_sessaoTipo, self).__init__(
            iface, 0, 0, st[self.current], [], title)
    ...
    def createPrismoid(self, j):
        if not self.prismoide.generate(j) is None:
            messageDialog(title="Erro", message="Falha ao criar o talude na estaca: "+str(
                self.progressiva[j]) + " estaca: "+prog2estacaStr(self.progressiva[j]))
        self.updateAreaLabels()

    def createLabels(self):
        self.banquetaLbC = pg.TextItem(
            text="Banqueta em aterro", color=(200, 200, 200), anchor=(.5, 0))
        ...
        self.banquetaLbA = pg.TextItem(
            text="Banqueta em corte", color=(200, 200, 200), anchor=(.5, 0))
    ...
    def perfil_grafico(self, reseting=False):
        ...
        if self.greide:
            ...
            self.roi = ssRoi(L, cota=self.verticais.getY(
                self.progressiva[self.current]))
    ...
    def updateLabels(self):
        ...
        self.banquetaLbC.setPos(self.roi.getHandlePos(
            0).x(), self.roi.getHandlePos(0).y())
        self.taludeLbC.setPos((self.roi.getHandlePos(2).x()+self.roi.getHandlePos(
            1).x())/2, (self.roi.getHandlePos(1).y()+self.roi.getHandlePos(2).y())/2)
        middle = float(self.st[self.current]
                       [int(len(self.st[self.current])/2)][1])
        self.pistaLb.setPos(0, middle)
        self.banquetaLbA.setPos(self.roi.getHandlePos(
            len(self.roi.handles)-1).x(), self.roi.getHandlePos(len(self.roi.handles)-1).y())
        self.taludeLbA.setPos((self.roi.getHandlePos(len(self.roi.handles)-3).x()+self.roi.getHandlePos(len(self.roi.handles)-2).x())/2,
                              (self.roi.getHandlePos(len(self.roi.handles)-3).y()+self.roi.getHandlePos(len(self.roi.handles)-2).y())/2)
...
#        if self.editMode:
    ...
    def setupUi(self, PerfilTrecho):
        ...
        self.btnExportDxf.setToolTip(
            "Exporta DXF que pode ser modificado e importado")
        ...
        self.btnAutoRange.setToolTip(
            "Ajusta o zoom e retorna para visualização padrão")
        ...
        self.selectEstacaComboBox.currentIndexChanged.connect(
            self.changeEstaca)
        ...
        self.applyBtn.setToolTip(
            "Aplica o desenho atual para um intervalo de estacas")
...
#        self.reset()
    ...
    def volumeCalc(self):
        try:
            ...
            msgLog(str(traceback.format_exception(None, e, e.__traceback__)))
            messageDialog(
                message="Erro! Os taludes definidos não encontram o terreno!")
    ...
    def changeEstaca(self):
        ...
        if not self.changingEstaca:
            self.current = int(self.progressiva.index(
    ...
    def reset(self):
        ...
        self.initVars(self.iface, 0, 0,
                      self.st[self.current], [], "Perfil Transversal")
        self.perfil_grafico()
        self.roi.sigRegionChangeFinished.connect(self.updateData)
    ...
    def updateAreaLabels(self):
        ...
        dist = Config.instance().DIST
        self.progressivaLb.setText("E: " + str(int(self.progressiva[self.current]/dist))+" + "+str(round(
            (self.progressiva[self.current]/dist-int(self.progressiva[self.current]/dist))*dist, 4)) + "  " + str(self.intersecTable[self.current][1]))
    ...
    def plotTransCurve(self):
        ...
        self.curve.clear()
        self.curve.setData(X, Y, pen=pg.mkPen(
            'b', width=4, style=QtCore.Qt.SolidLine))
    ...
    def applyTrans(self):
        ...
        if diag.exec_() == QtWidgets.QDialog.Accepted:
            ...
            for estaca in range(diag.progressivas[0], diag.progressivas[1]+1):
                ...
                newST = []
                for pt in st:
                    newST.append([float(pt[0]), float(
                        pt[1]) + float(greide.getY(progressiva))-float(greide.getY(self.progressiva[self.current]))])
...
#        self.btnCalcular.setDisabled(True)
        ...
        if not bruckData:
            ...
            # +self.roi.ymed))
            self.roi.setPos(
                QPointF(self.roi.pos().x(), float(bruckData[0][1])))
            self.updater()
...
#        self.updateLabels()
    ...
    def updater(self):
        if not self.roi.ismodifying:
            handles = [self.roi.getHandlePos(i).x()
                       for i in range(self.roi.countHandles())]
            self.setBruckData()
    ...
    def resetGeometry(self):
        reset = yesNoDialog(
            self, message="Realmente recalcular todo o Diagrama?")
        self.reseted = reset
        if reset:
            self.reset.emit()
    ...
    def csvExport(self):
        ...
        filename = QtWidgets.QFileDialog.getSaveFileName(
            filter="Arquivo csv(*" + filter + " *" + filter.upper() + ")")[0]
        if filename in ['', None]:
            return
        filename = str(filename) if str(filename).endswith(
