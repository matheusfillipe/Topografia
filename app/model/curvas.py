...
class Curvas(object):
    ...
    def get_curva_details(self, id_estaca=-1, id_curva=-1):
        ...
        con = sqlite3.connect(
            Config.instance().TMP_DIR_PATH+"tmp/data/data.db")
        if id_curva == -1:
            curva = con.execute("SELECT CURVA_id,g20,t,d,epi,epc,ept FROM CURVA_SIMPLES INNER JOIN CURVA ON CURVA_SIMPLES.CURVA_id=CURVA.id WHERE estaca_final_id=%d" % (
                id_estaca,)).fetchall()
        else:
            curva = con.execute(
                "SELECT CURVA_id,g20,t,d,epi,epc,ept FROM CURVA_SIMPLES INNER JOIN CURVA ON CURVA_SIMPLES.CURVA_id=CURVA.id WHERE CURVA.id=%d" % (id_curva,)).fetchall()
        con.close()
        compactZIP(Config.fileName)
        if len(curva) > 0:
            return curva[0]
    ...
    def gera_estacas_intermediarias(self, dist, epc, ept, g20):
        ...
        estaca_inicial_north = float(estaca_start[0][4])+(
            epc-float(estaca_start[0][3]))*math.cos(float(estaca_start[0][7])*math.pi/180)
        estaca_inicial_este = float(estaca_start[0][5])+(
            epc-float(estaca_start[0][3]))*math.sin(float(estaca_start[0][7])*math.pi/180)
        estaca_inicial = [
        ...
        for i in range(abs(int(estaca_end_vert-estaca_start_vert))):
            progressiva += dist
            est += 1
            deflexao = (float(dist)-estaca_start_sobra) * \
                (g20/40) if deflexao_acumulada == 0.0 else (g20/2)
            deflexao_acumulada += deflexao

            azimute = float(estaca_start[0][7])-deflexao_acumulada
            north = float(estaca_start[0][4])+(progressiva-float(
                estaca_start[0][3]))*math.cos(float(azimute)*math.pi/180)
            este = float(estaca_start[0][5])+(progressiva-float(
                estaca_start[0][3]))*math.sin(float(azimute)*math.pi/180)
            estaca = [
                '%d' % est,
                '',
                '%f' % progressiva,
        ...
        deflexao_acumulada += deflexao
        azimute = float(estaca_start[0][7])-deflexao_acumulada
        estaca_end_north = float(
            estaca_start[0][4])+(epc-float(estaca_start[0][3]))*math.cos(float(azimute)*math.pi/180)
        estaca_end_este = float(
            estaca_start[0][5])+(epc-float(estaca_start[0][3]))*math.sin(float(azimute)*math.pi/180)
    ...
    def gera_estacas(self, dist):
        ...
        for curva in curvas:
            ...
            estacas.extend(
                self.gera_estacas_intermediarias(dist, epc, ept, g20))
        return estacas
    ...
    def delete_curva(self, id, dados):
        extractZIP(Config.fileName)
        db = DB(Config.instance().TMP_DIR_PATH+"tmp/data/data.db",
                "CURVAS_DADOS", list(dados.keys()))
        db.apagarDado(id)
        compactZIP(Config.fileName)
        return True
    ...
    def edit(self, dados):
        ...
        id = [db.getDadoComId(i)['id'] for i in ids if db.getDadoComId(i)[
            'curva'] == dados["curva"]][-1]
        db.update(id, dados)
        compactZIP(Config.fileName)
    ...
    def getId(self, name, dados):
        ...
        db = DB(Config.instance().TMP_DIR_PATH + "tmp/data/data.db",
                "CURVAS_DADOS", list(dados.keys()))
        ids = db.acharDadoExato("file", self.id_filename)
        id = [db.getDadoComId(i)['id']
              for i in ids if db.getDado(i)['curva'] == name]
        dados = db.getDado(id[-1]) if id else dados
        compactZIP(Config.fileName)
        return id[-1] if id else False, dados

    def duplicate(self, newName, new_id, path):
        dados = ['file', 'tipo', 'curva', 'vel',
                 'emax', 'ls', 'R', 'fmax', 'D']
        extractZIP(Config.fileName)
        db = DB(Config.instance().TMP_DIR_PATH +
                "tmp/data/data.db", "CURVAS_DADOS", dados)
        ids = db.acharDadoExato('file', self.id_filename)
        ...
        for p in Path(path.parent).rglob("*"):
            shutil.copy(str(p), Config.instance().TMP_DIR_PATH +
                        "tmp/data/"+newName+"".join(p.suffixes))
        compactZIP(Config.fileName)
