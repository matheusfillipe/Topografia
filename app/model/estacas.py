...
from qgis._core import QgsPoint, QgsPointXY, QgsProject, QgsVectorFileWriter
...
from ..model.utils import (azimuth, diff, dircos, getElevation, length, msgLog,
                           p2QgsPoint, pairs, pointToWGS84, roundFloat2str)
...
class Estacas(object):
    ...
    def tipo(self):
        ...
        dataTopo = [0.0, proj[2], proj[2], proj[3], proj[3], proj[4]]
        con.close()
        compactZIP(Config.fileName)
    ...
    def deleteEstaca(self, idEstacaTable):
        ...
        p = Path(
            Config.instance().TMP_DIR_PATH + "tmp/data/" + str(idEstacaTable) + ".prism"
        )
        if p.is_file():
            p.unlink()
        p = Path(
            Config.instance().TMP_DIR_PATH + "tmp/data/" + str(idEstacaTable) + ".bruck"
        )
        if p.is_file():
            p.unlink()
        compactZIP(Config.fileName)
    ...
    def saveTrans(self, id_filename, prismoid: prismoide):
        ...
        con.close()
        prismoid.save(
            Config.instance().TMP_DIR_PATH + "tmp/data/" + str(id_filename) + ".prism"
        )
        compactZIP(Config.fileName)
    ...
    def getTrans(self, idEstacaTable):
        try:
            ...
            for e in est:
                table[0].append(
                    con.execute(
                        "SELECT y, cota FROM SESSAO_TIPO WHERE TRANSVERSAL_id = ?",
                        (e[0],),
                    ).fetchall()
                )
                table[1].append(
                    con.execute(
                        "SELECT y, cota FROM RELEVO_SESSAO WHERE TRANSVERSAL_id = ?",
                        (e[0],),
                    ).fetchall()
                )
                xList.append(e[1])
    ...
    def gera_vertice(self, isCalc):
        ...
        if (
            hasattr(self, "iface")
            and sum(1 for f in self.layer.getFeatures()) > 1
            and not isCalc
            and self.ask
        ):
        ...
        if ok:
            ...
            for elemento in self.layer.getFeatures():
                ...
                for seg_start, seg_end, tipo in pairs(elemento, self.estaca):
                    ...
                    az = azimuth(
                        ponto_inicial,
                        p2QgsPoint(
                            ponto_inicial.x() + ((self.distancia) * cosa),
                    ...
                    estacas_inteiras_sobra = (
                        estacas_inteiras + 1 if sobra > 0 else estacas_inteiras
                    )
                    if not sem_internet:
                        cota = getElevation(
                            crs,
                    ...
                    yield [
                        ...
                        cota,
                        az,
                    ], ponto_inicial, estacas_inteiras, prog, az, sobra, tamanho_da_linha, cosa, cosb, tipo, elemento, resto

                    prog += tamanho_da_linha
                    resto = (tamanho_da_linha - sobra) - (
                        self.distancia * estacas_inteiras
                    )
            ...
            i = int(int(i) + 1)
            cota = getElevation(
                crs, p2QgsPoint(float(ponto_final.x()), float(ponto_final.y()))
            )
            yield [
                "%d+%f" % (estaca, resto),
                "PI%d" % i,
                ...
                cota,
                az,
            ], ponto_final, 0, tamanho_da_linha, az, sobra, tamanho_da_linha, cosa, cosb, tipo, elemento, resto

    def gera_estaca_intermediaria(
    ...
    def create_estacas(self, isCalc=False, task=None):
        ...
        for (
            ...
            if desc in ["TS", "TC"]:  # Entrou em curva
                vertice[0], vertice[1] = str(estaca) + "+" + str(
                    resto
                ) if estaca else vertice[0], desc + str(vertexCount)
                ...
                vertice[0], vertice[1] = str(estaca) + "+" + str(
                    resto
                ) if estaca else vertice[0], "PI" + str(vertexCount)
                estacas.append(vertice)
                vertexCount += 1

            if (
                emCurva and curvaComputadaCount == curvaCount
            ):  # Estacas intermediárias internas em curvas
                e = 0
                for (
                    tmp_line,
                    progressiva,
                    ponto_anterior,
                ) in self.gera_estaca_intermediaria_curva(
                    feature.geometry(), estaca + 1, progressiva, sobra
                ):
            ...
            if not emCurva:  # Estacas intermediárias em tangentes
                if sobra > 0 and sobra < self.distancia and qtd_estacas > 0:
                    (
                        ...
                        progressiva,
                        ponto_anterior,
                    ) = self.gera_estaca_intermediaria(
                        estaca + 1, ponto_anterior, progressiva, az, cosa, cosb, sobra
                    )
                    estacas.append(tmp_line)
                    estaca += 1
                ...
                for e in range(qtd_estacas):
                    (
                        ...
                        estaca + e + 1, ponto_anterior, progressiva, az, cosa, cosb
                    )
                    estacas.append(tmp_line)
                estaca += qtd_estacas
    ...
    def removeGeoPackage(self, name):
        ...
        for path in Path(Config.instance().TMP_DIR_PATH + "tmp/data/").rglob(
            "*" + Config.RANDOM + name + ".gpkg*"
        ):
            path.unlink()
        for path in Path(Config.instance().TMP_DIR_PATH + "tmp/data/").rglob(
            name + ".gpkg*"
    ...
    def getSavedLayers(self, name):
        ...
        try:
            ...
            msgLog(
                "Erro ao remover diretório temporário: "
                + Config.instance().TMP_DIR_PATH
                + "    "
                + str(e)
            )
        ...
        R = [
            Path(self.saveLayerToPath(str(path.absolute())))
            for path in extractZIP(Config.fileName)
            if (
                str(path).endswith(".gpkg")
                or str(path).endswith(".gpkg-shm")
