...
def circleArc(layer, data, index, layer2, i):
    ...
    if index=="R":
        ...
        if data["C"]:
            ...
            E=abs(T*np.tan(np.deg2rad(data["D"]/4)))
            ...
            R,x,y=QgsGeometryUtils.circleCenterRadius(p1 ,p ,p2)
            pl2=qgsGeometryToPolyline(l2)
            p2=QgsPoint(QgsGeometryUtils.lineCircleIntersection(center=QgsPointXY(x,y),radius=R,linePoint1=pl2[0],linePoint2=pl2[-1], intersection=QgsPointXY(p2))[1])
        ...
        if data["C"]:
            ...
            p1=QgsPoint(l1.interpolate(l1.length()-data["T"]).asPoint())
            p2=QgsPoint(l2.interpolate(data["T"]).asPoint())
            data["C"]=True if index=="S" else data["C"]
            PI=QgsPoint(PI)
...
#    data["R"]=QgsGeometryUtils.circleCenterRadius(p1,p,p2)[0]
    ...
    layer.dataProvider().addFeatures([feat, f2])
    return data
