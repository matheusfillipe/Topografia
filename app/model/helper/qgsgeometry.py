...
from qgis.core import (QgsCircularString, QgsFeature, QgsGeometry, QgsPoint,
                       QgsPointXY, QgsRectangle, QgsVectorLayer)
...
def getTangentesGeometry(layer, i, j=0):
    ...
    if j == 0:
        ...
        G = [
            QgsGeometry.fromPolyline([p2QgsPoint(g1[0]), pi]),
            QgsGeometry.fromPolyline([pi, p2QgsPoint(g2[-1])]),
        ]
...
def deflection(layer2, i):
    ...
    a1, a2 = [
        azimuth(qgsGeometryToPolyline(l)[0], qgsGeometryToPolyline(l)[-1])
...
def polyTransCircle(layer, data, index, layer2, i, ic):
    ...
    p1 = p2QgsPoint(PI)
    p2 = p2QgsPoint(
        data["L"] * np.cos(np.deg2rad(angle)) + p1.x(),
    ...
    g1, g2 = (
        [g1, g2]
        if polyline(g1)[-1].distance(polyline(g2)[0])
        <= polyline(Ig1)[-1].distance(polyline(Ig2)[0])
        else [Ig1, Ig2]
    )
    ...
    E = abs(
        (data["R"] + p) / np.cos(np.deg2rad((data["D"] + 2 * np.rad2deg(theta)) / 2))
        - data["R"]
    )
    msgLog("E=" + str(E))
...
def circleArc(layer, data, index, layer2, i, ic):
    ...
    p2 = p2QgsPoint(
        corda * np.cos(np.deg2rad(angle)) + p1.x(),
    ...
    if index == "R":
        ...
        if data["C"]:
            if data["L"] > np.deg2rad(abs(d)) * data["R"] * abs(data["D"] / d):
                data["D"] = (
                    d
                    / abs(d)
                    * abs(abs(d) * data["L"] / (np.deg2rad(abs(d)) * data["R"]))
                )
        ...
        if data["C"]:
            ...
            l2.translate(dd.x(), dd.y())
            l2.rotate(
                abs((abs(d) - 180) / d * data["D"] + 180 - abs(d)) * d / abs(d) + d,
    ...
    if index == "R":
        ...
        if data["C"]:
            Lmax = np.sqrt(
                l1.length() ** 2
                + l2.length() ** 2
                - 2 * l1.length() * l2.length() * np.cos(np.deg2rad(360 - d))
            )
            if data["L"] > Lmax:
                data["L"] = Lmax
...
def tangent2(layer, data, index, layer2, i):
    ...
    if fcount > 0:
        ...
        lpt = p2QgsPoint(
            lpt.x() - length * np.cos(np.deg2rad(angle)),
