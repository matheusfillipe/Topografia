...
def getTangentesGeometry(layer, i, j=0):
    ...
    if j == 0:
        ...
        G = [QgsGeometry.fromPolyline(
            [p2QgsPoint(g1[0]), pi]), QgsGeometry.fromPolyline([pi, p2QgsPoint(g2[-1])])]
...
def deflection(layer2, i):
    ...
    a1, a2 = [azimuth(qgsGeometryToPolyline(
        l)[0], qgsGeometryToPolyline(l)[-1]) for l in [line1, line2]]
    a = a2-a1
    if abs(a) > 180.0:
        a = a/abs(a)*abs(180-a)
    return a
...
def polyTransCircle(layer, data, index, layer2, i, ic):
    ...
    if fcount > 0:
        ...
        if l1 is None:
            l1 = QgsGeometry(l2)
            dd = diff(qgsGeometryToPolyline(l2)[
                      0], qgsGeometryToPolyline(l2)[-1])
            l2.translate(dd.x(), dd.y())
        elif l2 is None:
            l2 = QgsGeometry(l1)
            dd = diff(qgsGeometryToPolyline(
                l1)[-1], qgsGeometryToPolyline(l2)[0])
            l2.translate(dd.x(), dd.y())
    ...
    p1 = p2QgsPoint(PI)
    p2 = p2QgsPoint(data["L"]*np.cos(np.deg2rad(angle)) +
                    p1.x(), data["L"]*np.sin(np.deg2rad(angle))+p1.y())
  ...
  #  if ((a2 - a1 >= 0 and (a1 <= 90 or a1 >= 270)) or (a2 - a1 <= 0 and (a1 >= 90 or a1 <= 270))) and not (
   #         (abs(a1 - a2) > 180 or (a1 < 90 and a2 > 90) or (a1 < 90, a2 < 90)) and not (a2 - a1) < 0):
    ...
    Ig1.rotate(polyline(l1)[0].azimuth(
        polyline(l1)[-1]) - 90, polyline(Ig1)[0])
    Ig2 = QgsGeometry.fromPolyline(IpointsT2)
    dd = diff(p2, IpointsT2[-1])
    Ig2.translate(dd.x(), dd.y())
    Ig2.rotate(polyline(l2)[0].azimuth(
        polyline(l2)[-1]) + 90, polyline(Ig2)[-1])
    ...
    E = abs(
        (data["R"]+p)/np.cos(np.deg2rad((data["D"]+2*np.rad2deg(theta))/2))-data["R"])
    msgLog("E="+str(E))
...
def circleArc(layer, data, index, layer2, i, ic):
    ...
    if index == "R":
        if data["C"]:
            ...
            l2.translate(dd.x(), dd.y())
            dd = diff(l2.interpolate(T).asPoint(),
        ...
        if data["C"]:
            if data["L"] > np.deg2rad(abs(d))*data["R"]*abs(data["D"]/d):
                data["D"] = d/abs(d)*abs(abs(d)*data["L"] /
                                         (np.deg2rad(abs(d))*data["R"]))
            if data["L"] > np.deg2rad(abs(d))*data["R"]:
                ...
                p1 = p2QgsPoint(l1.interpolate(
                    l1.length()-data["T"]).asPoint())
                p2 = p2QgsPoint(l2.interpolate(data["T"]).asPoint())
                PI = p2QgsPoint(PI)
        else:
        ...
        if data["C"]:
            ...
            l2.translate(dd.x(), dd.y())
            l2.rotate(
                abs((abs(d)-180)/d*data["D"]+180-abs(d))*d/abs(d)+d, qgsGeometryToPolyline(l2)[0])
            p = p2QgsPoint((p1.x()+p2.x())/2, (p2.y()+p1.y())/2)
            tmp_line = QgsGeometry.fromPolyline([p2QgsPoint(PI), p])
            p = p2QgsPoint(tmp_line.interpolate(E).asPoint())
            ...
            l3 = QgsGeometry(l2)
            dd = diff(qgsGeometryToPolyline(
                l3)[-1], qgsGeometryToPolyline(l3)[0])
...
def tangent(layer, data, index, layer2, i):
    ...
    if fcount > 0:
        ...
        l1 = fant.geometry()
        l2 = QgsGeometry(l1)
        dd = diff(qgsGeometryToPolyline(fant.geometry())
                  [-1], qgsGeometryToPolyline(fant.geometry())[0])
        l2.translate(dd.x(), dd.y())
    ...
    if index == "R":
        ...
        if data["C"]:
            Lmax = np.sqrt(l1.length()**2+l2.length()**2-2 *
                           l1.length()*l2.length()*np.cos(np.deg2rad(360-d)))
            if data["L"] > Lmax:
                data["L"] = Lmax
...
def tangent2(layer, data, index, layer2, i):
    ...
    if fcount > 0:
        ...
        lpt = p2QgsPoint(lpt.x()-length*np.cos(np.deg2rad(angle)),
                         lpt.y()-length*np.sin(np.deg2rad(angle)))

        l1 = QgsGeometry.fromPolyline(
            [p2QgsPoint(lpt), p2QgsPoint(qgsGeometryToPolyline(fant.geometry())[-1])])
        l2 = QgsGeometry(l1)
        dd = diff(qgsGeometryToPolyline(fant.geometry())
                  [-1], qgsGeometryToPolyline(fant.geometry())[0])
        l2.translate(dd.x(), dd.y())
