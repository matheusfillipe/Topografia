...
class DB():
    ...
    def _salvarDado(self, dado):
        ...
        self.cursor.execute("INSERT INTO "+self.tableName+" ("+str(self.dataNameList)[
                            1:-1] + ")VALUES (" + (len(self.dataNameList)*"?,")[:-1]+")", dado)
    ...
    def todosOsDadosComId(self):
        self.connect()
        dados = [self.toDictComId(row) for row in self.cursor.execute(
            "SELECT * FROM " + self.tableName)]
        self.close()
        return dados
