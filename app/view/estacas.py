...
from qgis.core import (QgsCoordinateTransform, QgsCoordinateTransformContext,
                       QgsFeature, QgsField, QgsFields, QgsGeometry, QgsPoint,
...
try:
    ...
    def _translate(context, text, disambig):
        return QtWidgets.QApplication.translate(
            context.encode("utf8"), text.encode("utf8"), disambig, _encoding
        )
    ...
    def _translate(context, text, disambig):
        return QtWidgets.QApplication.translate(
            context.encode("utf8"), text.encode("utf8"), disambig
        )


FORMESTACA1_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/Topo_dialog_estacas1.ui")
)
FORMGERATRACADO_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/Topo_dialog_gera_tracado_1.ui")
)
APLICAR_TRANSVERSAL_DIALOG, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/applyTransDiag.ui")
)
SETCTATI_DIALOG, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/setTransPtsIndexes.ui")
)
VERTICE_EDIT_DIALOG, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/Topo_dialog-cv.ui")
)
SET_ESCALA_DIALOG, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/setEscala.ui")
)
SELECT_FEATURE, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/selectFeature.ui")
)
ESTACAS_DIALOG, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/Topo_dialog_estacas.ui")
)
PROGRESS_DIALOG, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/progressBarDialog.ui")
)
BRUCKNER_SELECT, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/bruckner_select.ui")
)
VOLUME_DIALOG, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "../view/ui/volume.ui")
)
EXPORTAR_CORTE, _ = uic.loadUiType(
...
class CopySelectedCellsAction(QtWidgets.QAction):
    def __init__(self, iface, table_widget):
        if not isinstance(table_widget, QtWidgets.QTableWidget):
            raise ValueError(
                str(
                    "CopySelectedCellsAction must be initialised with a QTableWidget. A %s was given."
                    % type(table_widget)
                )
            )
        ...
        if len(self.table_widget.selectionModel().selectedIndexes()) > 0:
            ...
            matx = [
                [
                    None
                    for a in range(
                        max(columnIndex[-1] - columnIndex[0] + 1, nSelectedColumns)
                    )
                ]
                for a in range(max(1 + rowIndex[-1] - rowIndex[0], nSelectedRows))
            ]
...
class EstacasUI(QtWidgets.QDialog, FORMESTACA1_CLASS):
    ...
    def curvasDialog(self, estacas, layer):
        coordenadas = [
            [float(x), float(y)]
            for y, x in zip([e[3] for e in estacas], [e[4] for e in estacas])
        ].copy()
    ...
    def openCSV(self):
        ...
        if filename in ["", None]:
            return None
        fileDB, ok = QtWidgets.QInputDialog.getText(
            None,
            "Nome do arquivo",
            "Nome do arquivo a ser salvo no projeto:",
            text="Traçado csv",
        )
        if not ok:
            return None
    ...
    def new(
        self, recalcular=False, layerName=None, filename=None, lastIndex=0, ask=True
    ):
        ...
        if len(itens) == 0:
            ...
            messageDialog(
                message="Nenhuma layer visível foi encontrada no projeto, por favor adicione alguma."
            )
            return None
        ...
        if not filename:
            if not recalcular:
                filename = ""
                names = [
                    self.tableEstacas.item(i, 1).text()
                    for i in range(self.tableEstacas.rowCount())
                ]
                first = True
                ...
                while filename == "" or filename in names:
                    ...
                    filename, ok = QtWidgets.QInputDialog.getText(
                        None, "Nome do arquivo", "Nome do arquivo:", text=nomeSugerido
                    )
                    if not ok:
                        return None
        ...
        if layerName is not None:
            ...
            item, ok = QtWidgets.QInputDialog.getItem(
                None,
                "Camada com tracado",
                "Selecione a camada com o traçado:",
                reversed(itens),
                0,
                False,
            )
        ...
        if ask:
            estaca, ok = QtWidgets.QInputDialog.getInt(
                None, "Estaca Inicial", "Estaca Inicial:", 0, 0, 10000000, 2
            )
        else:
            ok = True
            estaca = 0
    ...
    def fill_table_index(self, files):
        ...
        for i, f in enumerate(files):
            ...
            for j, f2 in enumerate(f):
                tableItem = QtWidgets.QTableWidgetItem("%s" % f2)
    ...
    def create_line(self, p1, p2, name):
        ...
        line.setGeometry(
            QgsGeometry.fromPolyline(
                [
                    p2QgsPoint(
                        self.reprojectgeographic.transform(point=QgsPointXY(p1))
                    ),
                    p2QgsPoint(
                        self.reprojectgeographic.transform(point=QgsPointXY(p2))
                    ),
                ]
            )
        )
    ...
    def create_point(self, p1, name):
        ...
        point.setGeometry(
            QgsGeometry.fromPoint(
                p2QgsPoint(self.reprojectgeographic.transform(point=QgsPointXY(p1)))
            )
        )
    ...
    def gera_tracado_pontos(
        ...
        if inicial and final:
            ...
            self.callback(
                pontos=self.create_line(
                    p2QgsPoint(p1e, p1n), p2QgsPoint(p2e, p2n), "Diretriz"
                ),
                parte=1,
            )
    ...
    def resizeEvent(self, event):
        self.stretchTable(self.table)
        # Restores the original behaviour of the resize event
        super(EstacasUI, self).resizeEvent(event)
...
class EstacasIntersec(QtWidgets.QDialog):
    ...
    def fill_table(self, xxx_todo_changeme, f=False):
        ...
        self.tableWidget.setItem(k, 0, QtWidgets.QTableWidgetItem("%s" % estaca))
        self.tableWidget.setItem(k, 1, QtWidgets.QTableWidgetItem("%s" % descricao))
        self.tableWidget.setItem(k, 2, QtWidgets.QTableWidgetItem("%s" % progressiva))
        self.tableWidget.setItem(k, 3, QtWidgets.QTableWidgetItem("%s" % cota))
        """if not f:
...
class EstacasCv(QtWidgets.QDialog):
            ...
            if len(columnIndexes) == 0:
                self.tableWidget.setRangeSelected(
                    QTableWidgetSelectionRange(
                        row, 0, row, self.tableWidget.columnCount() - 1
                    ),
                    True,
                )
    ...
    def setIntersect(self):
        ...
        self.btnCorte.setToolTip(
            "Exportar curvas de nível do traçado como um arquivo dxf"
        )
        self.mode = "T"
        self.stretchTable()
    ...
    def setCv(self):
        ...
        try:
            ...
            pass
        self.tableWidget.setHorizontalHeaderLabels(
            ("Estaca", "Descrição", "Progressiva", "Greide")
        )
        self.btnGen.setText("Tabela de Interseção")
        self.btnCorte.setText("Exportar Greide")
        self.btnCorte.setToolTip(
            "Exportar o greide e o terreno do traçado como um arquivo dxf"
        )
        self.mode = "CV"
        self.stretchTable()
    ...
    def fill_table(self, estaca, f=False):
        ...
        for value in list(estaca):
            cell_item = QtWidgets.QTableWidgetItem("%s" % formatValue(value))
            cell_item.setFlags(cell_item.flags() ^ Qt.ItemIsEditable)
            self.tableWidget.setItem(k, j, cell_item)
            j += 1

        if len(estaca[1]) != 0 and not (
            estaca[1]
            in [self.comboBox.itemText(i) for i in range(self.comboBox.count())]
        ):
    ...
    def setupUi(self, Form):
        ...
        self.btnRecalcular.setWhatsThis(
            "Recalcula a tabela em vista \n A tabela vertical deve"
        ...
        # self.btnEstacas.clicked.connect(self.ref_super.tracado)
        self.btnBruck.setToolTip(
            "Visualizar e editar o diagrama de bruckner para um intervalo de estacas"
        )
        ...
        self.btnCrossSectionExport.setToolTip(
            "Extportar os contornos dos perfis transversais e verticais como um arquivo CAD dxf"
        )
        # self.btnEstacas.clicked.connect(self.ref_super.tracado)
        ...
        toolTip = (
            "<html><head/><body><p>Exportar um modelo tridimensional do projeto como um arquivo de malha triangular."
        ...
        self.comprimento.setWhatsThis(
            "Comprimento total do traçado (considerando a profundidade do greide)"
        )
    ...
    def openLayers(self):
        ...
        if l:
            ...
            l.renderer().symbol().setWidth(0.5)
    ...
    def zoom(self, row, column):
        ...
        layer = QgsVectorLayer(
            "Point?crs=%s" % (root.crs().authid()), "Estaca: " + str(e), "memory"
        )
...
class Estacas(QtWidgets.QDialog, ESTACAS_DIALOG):
    ...
    def __init__(self, iface, parent):
        ...
        self.comboBox.currentIndexChanged.connect(
            lambda: self.tableWidget.setRangeSelected(
                QTableWidgetSelectionRange(0, 0, 0, self.tableWidget.columnCount() - 1),
                True,
            )
        )
        self.comboBox.currentTextChanged.connect(self.search)
    ...
    def openTIFF(self):
        ...
        for i in range(mapCanvas.layerCount() - 1, -1, -1):
            try:
                layer = mapCanvas.layer(i)
                layerName = layer.name()
                if type(layer) == qgis._core.QgsRasterLayer and not layer.name() in [
                    "Google Terrain",
                    "Google Satellite",
                ]:
                    itens.append(layerName)
        ...
        item, ok = QtWidgets.QInputDialog.getItem(
            None,
            "Camada com tracado",
            "Selecione o raster com as elevações:",
            itens,
            0,
            False,
        )
    ...
    def setupUi(self, Form):
        ...
        self.tableWidget.setRowCount(0)
        self.tableWidget.setHorizontalHeaderLabels(
            ("Estaca", "Descrição", "Progressiva", "Norte", "Este", "Cota", "Azimute")
        )
        self.tableWidget.cellClicked.connect(self.zoom)
    ...
    def event(self, event: QtCore.QEvent):
        if (
            ...
            and self.windowState() & QtCore.Qt.WindowMinimized
            and hasattr(self, "chview")
        ):
            view = self.chview
            #                view.show()
            view.setWindowState(
...
class closeDialog(QtWidgets.QDialog):
    ...
    def setupUI(self):
        ...
        label = QtWidgets.QLabel("Deseja salvar suas alterações?")
        ...
        btnNot.setText("Não")
        btnNot.setToolTip("Descartar alterações")
...
class rampaDialog(QtWidgets.QDialog):
    def __init__(self, roi, segment, pos, index=1):
        ...
        self.setWindowTitle("Modificar Rampa")
        self.roi = roi
    ...
    def setupUI(self):
        ...
        InclLbl = QtWidgets.QLabel("Inclinação: ")
        posInclLbl = QtWidgets.QLabel("%")
        comprLbl = QtWidgets.QLabel("Distância inclinada: ")
        poscomprLbl = QtWidgets.QLabel("m")
        cotaLbl = QtWidgets.QLabel("Cotas: V" + str(self.index - 2) + "")
        cotaLbl2 = QtWidgets.QLabel("V" + str(self.index - 1))
        poscotaLbl = QtWidgets.QLabel("m")
        abscissalbl = QtWidgets.QLabel("Distância Horizontal: ")
        posabscissaLbl = QtWidgets.QLabel("m")
        ...
        H2layout.addItem(
            QtWidgets.QSpacerItem(
                80, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding
            )
        )
        ...
        self.compr = np.sqrt(
            (h2.pos().y() - h1.pos().y()) ** 2 + (h2.pos().x() - h1.pos().x()) ** 2
        )
        self.cotaText = cota
    ...
    def updateCota(self):
        try:
            if not self.isBeingModified:
                ...
                self.update()
                self.compr = np.sqrt(
                    (self.h2.pos().y() - self.h1.pos().y()) ** 2
                    + (self.h2.pos().x() - self.h1.pos().x()) ** 2
                )
                self.Incl = (
                    100
                    * (self.h2.pos().y() - self.h1.pos().y())
                    / (self.h2.pos().x() - self.h1.pos().x())
                )
                self.redefineUI(2)
        except ValueError:
            pass
...
class ssRampaDialog(rampaDialog):
    ...
    def setupUI(self):
        ...
        compr.setMaximum(1000000000.0)
        compr.setMinimum(0.0)
        compr.setSingleStep(0.1)
        cota = QtWidgets.QDoubleSpinBox()
        cota.setMinimum(-10000.0)
        cota.setMaximum(10000.0)
        cota.setSingleStep(0.1)
        cota2 = QtWidgets.QDoubleSpinBox()
        cota2.setMinimum(-10000.0)
        cota2.setMaximum(10000.0)
        cota2.setSingleStep(0.1)
        abscissa = QtWidgets.QDoubleSpinBox()
        abscissa.setMaximum(100000.0)
        abscissa.setMinimum(-10000.0)
        abscissa.setSingleStep(0.1)

        InclLbl = QtWidgets.QLabel("Inclinação: ")
        posInclLbl = QtWidgets.QLabel("%")
        comprLbl = QtWidgets.QLabel("Comprimento: ")
        poscomprLbl = QtWidgets.QLabel("m")
        cotaLbl = QtWidgets.QLabel("Cota:      ")
        poscotaLbl = QtWidgets.QLabel("m")
        abscissalbl = QtWidgets.QLabel("Distância até o eixo")
        posabscissaLbl = QtWidgets.QLabel("m")
    ...
    def updateCompr(self):
        try:
            if not self.isBeingModified:
                ...
                self.abscissa = round(
                    self.abscissa + np.cos(np.deg2rad(self.Incl)) * dc, 2
                )
                self.update()
                self.redefineUI(1)
...
class ApplyTransDialog(QtWidgets.QDialog, APLICAR_TRANSVERSAL_DIALOG):
    ...
    def setupUi2(self):
        d = self.dist
        self.firstCb.addItems(
            list(map(lambda i: fastProg2EstacaStr(i, d), self.progressiva))
        )
        self.setSecondCb()
    ...
    def setSecondCb(self):
        d = self.dist
        self.secondCb.clear()
        self.secondCb.addItems(
            list(
                map(
                    lambda i: fastProg2EstacaStr(i, d),
                    self.progressiva[self.firstCb.currentIndex() :],
                )
            )
        )
        self.setIndexes()
    ...
    def __init__(self, iface, layer):
        ...
        for feat in layer.getFeatures():
            ...
            if feat.hasGeometry():
                ...
                for r in row:
                    ...
                    item = QtWidgets.QTableWidgetItem("%s" % r)
        ...
        self.tableWidget.setCurrentIndex(
            self.tableWidget.model().index(
                self.tableWidget.rowCount() - 1, self.tableWidget.columnCount() - 1
            )
        )
